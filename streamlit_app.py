...
if audio_input:
    if st.button("🔍 Biyometrik Analizi Başlat", type="primary", use_container_width=True):
        ...
        with st.spinner("Ses imzanız yapay zeka ile analiz ediliyor..."):
            audio_bytes = audio_input.read()
            
            # Librosa ile teknik ölçümler
            ...
            try:
                ...
                prompt = f"Bu ses tonunun teknik verileri: Ortalama Frekans: {mean_pitch:.1f} Hz, Enerji (RMS): {rms:.4f}, Jitter: {jitter}. Bu verileri ve sesin genel tonunu dikkate alarak kişinin o anki zindelik, enerji seviyesi ve yorgunluk durumu hakkında bütüncül, samimi ve psikolojik/fiziksel bir değerlendirme yapar mısın?"
                ...
                ai_comment = "Yapay zeka analizi sırasında geçici bir bağlantı oluştu, ancak metrikleriniz hesaplandı."

            st.session_state.analysis_results = {
                "rms": rms, "pitch": mean_pitch, "jitter": jitter, 
                "name": name, "col": col, "icon": icon, "ai_comment": ai_comment
            }
            st.rerun()
...
if st.session_state.analysis_results:
    ...
    st.markdown("#### 🧠 Yapay Zeka Ses Değerlendirmesi")
